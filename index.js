(function (global, factory) {
    ...
    function playPause(form) {
        ...
        var path = this.util.addIcon.call(
            this,
            button,
            (this.settings.play ? faPause : faPlay).icon
        );
        button.on(
            'click',
            function () {
    ...
    function ticker() {
        ...
        var now = this.layout.svg
            .append('text')
            .attr('text-anchor', 'end')
            .attr('x', this.settings.width - 6)
            .attr(
                'y',
                this.settings.margin.top + this.scale.y.bandwidth() * (this.set.id.size - 0.45)
            )
            .attr('dy', '0.32em')
